        ...
        try:
            ...
            if history:
                ...
                for h in recent:
                    ...
                    lines.append(
                        f"{h.get('report_date', '?')} 评{h.get('weighted_score', '?')}分{dir_cn}"
                        f" → 10日{h.get('return_10d', 0):+.1f}%"
                    )
                candidates.append({
                    "type": "该股历史",
                    "priority": 10,
...
要求：
1. 优先选择：该股历史、校准警示、跨股盘感、相似案例教训（这些不可替代）
2. 选择性纳入：与当前股票技术形态/板块/环境相关的信念、智慧、情报
3. 如果候选池中有矛盾信息（如看多vs看空），都要保留并标注分歧
4. 输出格式：用 ▸ 开头的条目列表，每条 1-2 行，按重要性排序
5. 末尾加一行："⚠️ 以上为历史知识参考，样本量有限时仅供辅助判断"
6. 总长度控制在 1500-3000 字，信息少时可以更短
"""
