...
# For history, get last 30 days to calculate gains
start_date = (datetime.datetime.now() - datetime.timedelta(days=40)).strftime('%Y%m%d')

print(f"Scouting {ts_code} via Tushare...")

try:
    # 1. Basic Info & Valuation
    df_basic = pro.daily_basic(ts_code=ts_code, trade_date='', limit=1)
    ...
    if not df_daily.empty:
        ...
        amount = df_daily.iloc[-1]['amount'] / 10000 # Convert to 亿 if needed, but Tushare 'amount' is in thousand RMB
