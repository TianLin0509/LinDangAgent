...
def get_600436_data():
    ...
    try:
        ...
        if not row.empty:
            ...
            # Historical for gains
            hist_df = ak.stock_zh_a_hist(symbol="600436", period="daily", adjust="qfq")
            ...
            # Capital flow
            flow_df = ak.stock_individual_fund_flow(stock="600436", market="sh")
            if not flow_df.empty:
                latest_flow = flow_df.iloc[-1]
                print(f"Main Net Inflow: {latest_flow['主力净流入']} 元")
        else:
            print("Stock 600436 not found in spot data.")
